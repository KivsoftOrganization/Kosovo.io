...
class Client {
    ...
    createUUID() {
        ...
        localStorage.setItem('Kiv3UUID', token);
        return token;
    }
    ...
    loadCSS() {
        ...
        if (!localStorage.getItem('Kiv3UUID')) localStorage.setItem('Kiv3UUID', this.createUUID());
        this.uuid = localStorage.getItem('Kiv3UUID');
    }

    options(){
        var a = prompt("Bot name? ", "");
        var b = ""
        while (true){
            var b = prompt("Amount of bots?", "");
            if(/^\d+$/.test(b)) {
                break;
            }
            else {
                alert("Only type numbers here.")
            }
        }
        string1 = a
        string2 = b
        localStorage.setItem('botAmount', b);
        localStorage.setItem('botNick', a);
    }

    loadGUI() {
        document.title = "Kosovo.io";
        //if (localStorage.getItem("botAmount") === null || localStorage.getItem("botNick") === null)
        //{
            window.client.options()
        //}
        $("#title").replaceWith('<div data-v-0733aa78="" id="title" class="guest">Kosovo.io</div><br><h4>hacks made by kivsoft</h4>')
        $('#mainui-offers').replaceWith('<div style="  position: absolute;top: 50px;width:500px;height:100px;border:1px solid #000;"><button onclick="window.client.startBots(localStorage.getItem(\'botAmount\'));">Start Bots</button><br><button onclick="window.client.stopBots();">Stop bots</button><br><button onclick="window.client.options()">Change name of bots and amount</button></div>')

        // <img src="https://media.discordapp.net/attachments/804416406699376742/804421967082946636/realmr2kgod-20210122-0001.jpg?width=262&amp;height=500">

        $(".agario-promo").replaceWith('<div class="agario-promo"><img style="height: 100%; width: 100%; object-fit: contain" src="https://media.discordapp.net/attachments/804416406699376742/804422370147303484/kaulopistolo-20210117-0001.jpg?width=662&height=662" alt="" usemap="#loggedout"></div>')
        console.log('shit loaded');
    }

    startBots(amount) {
        
        localStorage.setItem('botAmount', string2);
        localStorage.setItem('botNick', string1);

        var isMobileVersion = document.getElementsByClassName('form-control party-token');
        if (!isMobileVersion.length > 0) {
            alert("you need to make or join a party")
            return
        }
        ...
        console.log(`starting ${localStorage.getItem('botAmount')} amount of bots!`);
    ...
    onMessage(msg) {
        ...
        switch (msg.getUint8(offset++)) {
            ...
            case 242:
                ...
                window.agarApp.recaptcha.requestCaptchaV3('play', token => this.spawn(localStorage.getItem('botNick') + 'x', token));
                break;
